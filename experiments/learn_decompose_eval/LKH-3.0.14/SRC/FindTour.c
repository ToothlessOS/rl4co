...
{
    GainType Cost, BetterRouteScore = 0, BetterRouteCost = 0;
    Node *t;
    int BetterRouteNodes = 0, i;
    ...
    if (MaxTrials > 0) {
        ...
        BetterRouteCost = PLUS_INFINITY;
        BetterRouteScore = PLUS_INFINITY;
        ...
        BetterRouteNodes = RouteNodes;
        BetterRouteScore = RouteScore;
        BetterRouteCost = RouteCost;
    }
    for (Trial = 1; Trial <= MaxTrials; Trial++) {
        ...
        if (CurrentPenalty < BetterPenalty ||
            ...
            BetterRouteCost = RouteCost;
            RecordBetterTour();
            /* LDE patch: write current best (within-run) tour on every improvement
               so an external orchestrator can read intermediate solutions during
               the search. The OUTPUT_TOUR_FILE write below still happens only on
               global-best. */
            if (IntermediateTourFileName)
                WriteTour(IntermediateTourFileName, BetterTour, BetterCost);
    ...
    RouteNodes = BetterRouteNodes;
    RouteScore = BetterRouteScore;
    RouteCost = BetterRouteCost;
    return BetterCost;
}
