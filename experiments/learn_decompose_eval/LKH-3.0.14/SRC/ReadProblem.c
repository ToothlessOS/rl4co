 ...
 *
 * NODE_SCORE_SECTION :
 * The scores of all nodes of an OP, PTP, or PC-TSP  are given in the form
 * (per line)
 *
 *    <integer> <integer>
 *
...
void ReadProblem(void)
{
    ...
    if (ProblemType != CVRP && ProblemType != CVRPTW &&
        ...
        ProblemType != KTSP && ProblemType != OKTSP &&
        ProblemType != CluVRP && ProblemType != OP &&
        ProblemType != SoftCluVRP && ProblemType != GCTSP &&
        ProblemType != CCCTSP && ProblemType != MSCTSP &&
        ProblemType != PTP && ProblemType != TSPMD &&
        ProblemType != PCTSP && ProblemType != PC_TSP) {
        M = INT_MAX / 2 / Precision;
        for (i = Dim + 1; i <= DimensionSaved; i++) {
